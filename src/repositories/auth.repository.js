...
export async function CreateUser(username, profileUrl, email, hash) {
  const result = await db.query(
    `INSERT INTO users("userName", "profileUrl", email, hash) VALUES ($1, $2, $3, $4) RETURNING id`,
    [username, profileUrl, email, hash]
  );
...
export async function CreateSession(userId) {
  await db.query(`INSERT INTO sessions("userId", token) VALUES ($1, null)`, [
    userId,
  ]);
}
...
export async function UpdateSession(userId, token) {
  await db.query(`UPDATE sessions SET token = $2 WHERE "userId" = $1`, [
    userId,
    token,
  ]);
}
